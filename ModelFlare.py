...
def parameters_guess(lightcurve, fl_indx):
    ...
    offset = np.nanmedian(lightcurve['mag'].values[isflare])
    amp = offset - np.nanmin(lightcurve['mag'].values[fl_indx])
...
def fit_gauss(lightcurve, fl_indx):
    ...
    para_gauss, cov_gauss = curve_fit(ms.gaus, lightcurve['mjd'].values[isflare],lightcurve['mag'].values[isflare], 
...
def fit_flaremodel(lightcurve, fl_indx):
    offset, amp, sigma, timepeak, isflare = parameters_guess(lightcurve, fl_indx)
    err = lightcurve['magerr'].values[isflare]
    para_flare, cov_flare = curve_fit(ms.aflare1, lightcurve['mjd'].values[isflare],lightcurve['mag'].values[isflare], 
                                      p0=[amp, offset, timepeak, sigma], sigma=err)
...
def chisquared(mag, magerr, fitmag, para):
    '''
...
def AIC(mag, magerr, fitmag, para):
    k = len(para)
    n = len(mag)
    loglike = -1 * np.sum((mag - fitmag) ** 2 / (2 * magerr ** 2))
    AIC = (2 * k) - (2 * loglike)
    return AIC
...
def plot_models(file, lightcurve, fl_indx, ids, initout_csv, residuals=False, stats=None, time=None, full_lc=False, save=False):
    ...
    ax.errorbar(lightcurve['mjd'].values[fl_indx],lightcurve['mag'].values[fl_indx],lightcurve['magerr'].values[fl_indx], 
...
def runmatch(matchfile, initout_csv):
    ...
    # loads the full data from the a matchfile
    sources, sourcedata, transients, transientdata = matchflare.loaddata(matchfile)
    # from initout_csv identify which lightcurves are from the sourcedata and 
    # which is from transientdata
    match_sources = initout.loc[initout['type'] == 'sources']
    match_transients = initout.loc[initout['type'] == 'transients']
